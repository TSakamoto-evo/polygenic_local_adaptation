...
Population::Population(const Parameter input_para, int& generation){
  ...
  std::vector<int> tmp_order_island(2 * para.pop_size_island);
  order_island = tmp_order_island;
  std::iota(order_island.begin(), order_island.end(), 0);

  std::vector<int> tmp_order_continent(2 * para.pop_size_continent);
  order_continent = tmp_order_continent;
  std::iota(order_continent.begin(), order_continent.end(), 0);
...
void Population::ret_reco_genotype_island(const int parent,
  ...
  // determine which allele is inherited at the focal locus
  bool which1 = p(mt);
  ...
  parent_island[parent].ret_haplotype(ret_small, ret_major, reco_pos, which1, which2);
}
...
void Population::ret_reco_genotype_continent(const int parent,
  ...
  while(start < para.num_loci){
   ...
   parent_continent[parent].ret_haplotype(ret_small, ret_major, reco_pos, which1, which2);
}
...
void Population::selection(){
  ...
  std::shuffle(order_island.begin(), order_island.end(), mt);
  std::shuffle(order_continent.begin(), order_continent.end(), mt);

  std::vector<double> geno_small(para.num_loci);
  double geno_major = 0.0;

  // reproduction in the island population
  int mig_num_island = det_mig(mt);

  // parent is a migrant from the continent population 
  for(int i = 0; i < mig_num_island; i++){
    int ind = order_island[i] / 2;
    bool which = order_island[i] % 2;
  ...
  for(int i = mig_num_island; i < 2 * para.pop_size_island; i++){
    int ind = order_island[i] / 2;
    bool which = order_island[i] % 2;
  ...
  // reproduction in the continent population
  int mig_num_continent = det_mig(mt);

  // parent is a migrant from the island population 
  for(int i = 0; i < mig_num_continent; i++){
    int ind = order_continent[i] / 2;
    bool which = order_continent[i] % 2;
  ...
  // parent is a nonmigrant
  for(int i = mig_num_continent; i < 2 * para.pop_size_continent; i++){
    int ind = order_continent[i] / 2;
    bool which = order_continent[i] % 2;
